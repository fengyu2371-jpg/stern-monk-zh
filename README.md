# stern-monk-zh-tw v19｜單一面板與十分鐘鎖定版
...
玩家按下主面板功能後，修士會直接編輯原本那則面板訊息，不再於頻道中另外發出學生資料、城下町、神諭冊或教學訊息。

### 操作工作階段

- 第一位開啟功能的成員會暫時取得面板操作權。
- 畫面仍可讓所有成員查看。
- 只有開啟者可以操作目前畫面的按鈕與選單。
- 其他玩家按下時只會收到私密提示，不會跳出自己的資料頁，也不會改動目前畫面。
- 每個操作頁都有「返回修士面板」，可主動釋放面板。

### 10 分鐘鎖定

- 以最後一次有效操作重新計時。
- 連續 10 分鐘沒有操作，舊畫面會自動鎖定。
- 同一則訊息會恢復為公共修士主面板，其他成員可重新使用。

### 表單結果

入學登記、修改學籍、神諭偏好、地點登記與教學問題仍使用 Discord 表單；送出後會更新原本的修士面板訊息，不另發公開結果訊息。

告解內容與回覆涉及隱私，因此仍是私密回覆；完成後會釋放面板工作階段。
